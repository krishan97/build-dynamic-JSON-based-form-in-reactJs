...
export const RadioBoxField = (data) => {
            ...
            <data.Form.Group>
                ...
                {data.field.options.map((x, index) => (
                    <data.Form.Check key={index} className={'p-2'} name={data.field.name} inline
                     label={x.label} value={x.value}  defaultChecked={(data.field.value ===x.value)?  true: false}
                     type={data.field.type} id={`inline-${data.field.name}-${data.field.type}-${index}-${index}`} onClick={() => data.handleClick(data.index, x.value)} />
                ))}
